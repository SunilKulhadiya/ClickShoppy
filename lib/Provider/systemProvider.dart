...
import 'package:clickshoppy/Screen/Payment/Widget/PaymentRadio.dart';
import 'package:clickshoppy/Screen/StripeService/Stripe_Service.dart';
import 'package:clickshoppy/Provider/SettingProvider.dart';
import 'package:clickshoppy/repository/paymentMethodRepository.dart';
import 'package:clickshoppy/repository/systemRepository.dart';
...
class SystemProvider extends ChangeNotifier {
  ...
  Future getCurrency({curLng}) async {
    print("@@@@@@@@@@@@@@@@@ systemProvider.dart @@@@@@@@@@@@@@@@@@@@@@@@@@@  getCurrency @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ${curLng}");
    try {
      Map<String, dynamic> parameter = {};
      var data =
      await SystemRepository.fetchSystemSetting(parameter: parameter);
      print("********  systemProvider.dart, ---- data : ${data['systemSetting']['system_settings'][0]}");
      var Currncy = "";
      if (!data['error']) {
        var systemData = data['systemSetting']['system_settings'][0];
        print("@@@@@@@@@@@@@@@@@@@@@@@@@ systemProvider.dart @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ${systemData['currency']}");

        if(curLng == 'English') {
          Currncy = systemData['currency'];
        }else{
          if(systemData['mother_tongue'] == null){
            Currncy = systemData['currency'];
          }else{
            Currncy = systemData['mother_tongue'];
          }
        }
        return Currncy;
      }else {
  ...
  Future<Map<String, dynamic>> getSystemSettings({String? userID}) async {
    try {
      ...
      if (!data['error']) {
        ...
        CUR_CURRENCY = currentLanguage == 'en' ? systemData['currency'] : systemData['mother_tongue']==null ?
                        systemData['currency'] : systemData['mother_tongue'];
